...
echo""
echo "                   ___                                     "
echo "                  /   | ______________  _____(_)___ _____  "
echo "                 / /| |/ ___/ ___/ __ \/ ___/ / __ '/ __ \ "
echo "                / ___ / /__/ /__/ /_/ / /  / / /_/ / / / / "
echo "               /_/  |_\___/\___/\____/_/  /_/\__,_/_/ /_/  "
echo "                                                           "
echo "                      +-+-+-+-+-+ +-+-+-+-+-+-+-+-+	    "
echo "                      |b|r|a|n|d| |s|e|c|u|r|i|t|y|	    "
echo "                      +-+-+-+-+-+ +-+-+-+-+-+-+-+-+	    "
echo ""
...
echo "[+] Ruinning Amass..."
#	amass enum --passive -d $domain | awk -F ' ' '{print $1}' | tee ~/projects/$seed/$domain/domains/amass.txt
...
echo "[+] Gathering waybackurls from Time-Machine..."
	cat ~/projects/$seed/$domain/domains/probed.txt | gau --subs -bf ttf,woff,svg,png | tee ~/projects/$seed/$domain/scans/waybackurls.txt
...
echo "[+] Scanning with Nuclei..."
	...
	nuclei -t ~/tools/nuclei-templates/ -silent -l ~/projects/$seed/$domain/domains/probed.txt -o ~/projects/$seed/$domain/vulnerabilities/nuclei -H X-Testing-By:accorian.com
...
echo "[+] Checking with Jira-Lens..."
        ...
        python3 ~/tools/Jira-Lens/Jira-Lens/Jira-Lens.py -f ~/projects/$domain/domains/probed.txt -o ~/projects/$domain/domains/vulnerabilities/$domain
        cd

echo "[+] Scanning with Jaeles..."
	jaeles config init
	jaeles scan -s ~/.jaeles/base-signatures  -U ~/projects/$domain/domains/probed.txt -c 50 -o ~/projects/$seed/$domain/vulnerabilities/jaeles
